        ...
        index: function () {
            ...
            //加载页面自动请求当前是否有新任务数据
						$(document).ready(function(){
							$.get("https://cat.avantgared.com/KIldJsDvMC.php/totalorder/orderlist",{},function(result){
								if (JSON.stringify(result.data)) {
									if (JSON.stringify(result.data['list'])=="null" || JSON.stringify(result.data['list'])==null) {
										$('#order').css('display','none');
									}else{
										$('#order').css('display','block');
										$('#order').css('position','absolute');
										$('#order').css('width','200px');
										$('#order').css('height','100%');
										$('#order').css('background','#f39c12');
										$('#order').css('right','400px');
										$('#order').css('color','#fff');
										$('#order').css('font-weight','bolder');
										$('#demo1').text(JSON.stringify(result.data['list']));
										$(function () {
											var demo = $("#demo")[0];
											var demo1 = $("#demo1")[0];
											var demo2 = $("#demo2")[0];
									...
									if (JSON.stringify(result.data['data'])=="null" || JSON.stringify(result.data['data'])==null) {
										$('#faultdeclaration').css('display','none');
									}else{
										$('#demo3').text(JSON.stringify(result.data['data']));
										$('#faultdeclaration').css('display','block');
										if (JSON.stringify(result.data['list'])=="null" || JSON.stringify(result.data['list'])==null) {
											$('#faultdeclaration').css('position','absolute');
											$('#faultdeclaration').css('width','200px');
											$('#faultdeclaration').css('height','100%');
											$('#faultdeclaration').css('background','#f39c12');
											$('#faultdeclaration').css('right','400px');
											$('#faultdeclaration').css('color','#fff');
											$('#faultdeclaration').css('font-weight','bolder');
										}else{
											$('#faultdeclaration').css('position','absolute');
											$('#faultdeclaration').css('width','200px');
											$('#faultdeclaration').css('height','100%');
											$('#faultdeclaration').css('background','#f39c12');
											$('#faultdeclaration').css('right','610px');
											$('#faultdeclaration').css('color','#fff');
											$('#faultdeclaration').css('font-weight','bolder');
										}
										$(function () {
											...
											var demo4 = $("#demo4")[0];
											var speed = 100;    //滚动速度值，值越大速度越慢
											var nnn = 200 / demo3.offsetHeight;
											for (i = 0; i < nnn; i++) {
													demo3.innerHTML += "<br />" + demo3.innerHTML
											}
											demo4.innerHTML = demo3.innerHTML    //克隆demo4为demo3
											function Marquee() {
												if (demo4.offsetTop - demo5.scrollTop <= 0)    //当滚动至demo3与demo4交界时
													demo5.scrollTop -= demo3.offsetHeight    //demo跳到最顶端
												else {
													demo5.scrollTop++
												}
											}
											...
											var MyMar = setInterval(Marquee, speed);        //设置定时器
											demo5.onmouseover = function () {
												clearInterval(MyMar)
											}    //鼠标经过时清除定时器达到滚动停止的目的
											demo5.onmouseout = function () {
												MyMar = setInterval(Marquee, speed)
											}    //鼠标移开时重设定时器
									});
									}
