...
class Withdrawal extends Backend
{
        ...
        if ($this->request->isAjax()) {
            ...
            if (session('venue_id')) {
                $list = $this->model
                        ->with(['venue'])
                        ->where($where)
                        ->where('withdrawal.venue_id',session('venue_id'))
                        ->order($sort, $order)
                        ->paginate($limit);
            }else{
                $list = $this->model
            ...
            $result = array("total" => $list->total(), "rows" => $list->items(), "extend" => ['venue_id' => session('venue_id')??0]);
    ...
    public function examine($ids=''){
			...
			if ($this->request->isPost()) {
					...
					$this->success('打款成功','/KIldJsDvMC.php/withdrawal?ref=addtabs');
			}
	...
	public function remark($ids='')
	{
			...
			if ($this->request->isPost()) {
				...
				try {
						$data['status'] = '30';
						$before = db('venue')->where('id',$row['venue_id'])->value('money');
						$this->model->where('id',$ids)->update($data);
						$this->venuelog->addvenuemongylog($row['venue_id'],$row['money'],$before,'场馆提现驳回退款','80');
						// 提交事务
						Db::commit();
				} catch (\Exception $e) {
						// 回滚事务
				...
				$this->success('操作成功','/KIldJsDvMC.php/withdrawal?ref=addtabs');
		 }
			$this->view->assign("row", $row);
			return $this->view->fetch();
	}
